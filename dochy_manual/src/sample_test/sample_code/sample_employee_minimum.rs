...
fn sample_employee_minimum() -> DpResult<()>{
    ...
    let mut ml : MListPtr<MItemPtr> = dochy::core::intf::root::get_mlist(
        rp, "@Employees")?;
    let item = ml.insert();
    mitem_ptr::set_str(item, "userId", Qv::Val("krish".to_string()));
    mitem_ptr::set_str(item, "jobTitle", Qv::Val("Developer".to_string()));
    mitem_ptr::set_str(item, "firstName", Qv::Val("Krish".to_string()));
    mitem_ptr::set_str(item, "lastName", Qv::Val("Lee".to_string()));
    mitem_ptr::set_str(item, "employeeCode", Qv::Val("E1".to_string()));
    mitem_ptr::set_str(item, "region", Qv::Val("CA".to_string()));
    mitem_ptr::set_str(item, "phoneNumber", Qv::Val("123456".to_string()));
