...
mod tests {
    ...
    fn test_diff3() -> Result<(), String>{
        ...
        let diff1 = dochy_diff::get_diff(&applied1, unsafe{ intf.root_obj_ref() }).or_else(|e| Err(e.to_string()))?;
        ...
        let diff2 = dochy_diff::get_diff(&applied1, unsafe{ intf.root_obj_ref() }).or_else(|e| Err(e.to_string()))?;
